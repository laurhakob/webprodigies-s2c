...
import { Hash, Type, LayoutIcon } from "lucide-react";
...
const Layout = ({ children }: Props) => {
  return (
    <Tabs defaultValue="colours" className="w-full">
              ...
              {tabs.map((tab) => {
                ...
                return (
                  <TabsTrigger
                    key={tab.value}
                    value={tab.value}
                    className="flex items-center gap-2 rounded-xl data-[state=active]:bg-white/15 data-[state=active]:backdrop-blur-xl data-[state=active]:border data-[state=active]:border-white/20 transition-all duration-200 text-xs sm:text-sm"
                  >
      ...
      {/* This was missing — renders style-guide/page.tsx */}
