...
export const useInfiniteCanvas = () => {
  ...
  const hasSelectedText = Object.keys(shapesEntities).some((id) => {
  ...
  const onPointerMove: React.PointerEventHandler<HTMLDivElement> = (e) => {
    ...
    if (isErasingRef.current && currentTool === "eraser") {
      ...
      if (hitShape && !erasedShapesRef.current.has(hitShape.id)) {
        // Delete shape only once during this drag
        dispatch(removeShape(hitShape.id));
        erasedShapesRef.current.add(hitShape.id);
      }
  ...
  const onKeyDown = (e: KeyboardEvent): void => {
    if ((e.code === "ShiftLeft" || e.code === "ShiftRight") && !e.repeat) {
      e.preventDefault();
      isSpacePressed.current = true; // Keep the same ref name
  ...
  const attachCanvasRef = (ref: HTMLDivElement | null): void => {
    // Clean up wheel listener from old canvas
    ...
    // Add wheel listener to new canvas
    if (ref) {
