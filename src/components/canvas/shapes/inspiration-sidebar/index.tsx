...
const InspirationSidebar = ({ isOpen, onClose }: InspirationSidebarProps) => {
  ...
  const uploadImage = useCallback(
    async (file: File): Promise<{ storageId: string }> => {
      try {
        ...
        // Step 3: Save reference (if project exists)
        if (projectId) {
  ...
  return (
    <div
      className={cn(
        "fixed left-5 top-1/2 -translate-y-1/2 w-80 backdrop-blur-xl bg-white/8 border border-white/12 gap-2 p-3 saturate-150 rounded-lg z-50 transition-transform duration-300"
